...
class HelpSession:
	...
	def buildUsage(self):
		self.usage = "Usage: {} command<br>Normal Commands:".format(self.pearl.config['format'])
		for command in self.config['commands']:
			self.usage += '<br><b>{}</b>: {}'.format(command, self.config['commands'][command])

	def buildHandle(self):
		messageFilter = nacre.handle.newMessageFilter('^{}+help(\s.*)?$'.format(self.pearl.config['format']))
